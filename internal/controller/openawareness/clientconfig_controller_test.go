...
var _ = Describe("ClientConfig Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ClientConfig")
			err := testClient.Get(ctx, typeNamespacedName, clientconfig)
			if err != nil && errors.IsNotFound(err) {
				resource := &openawarenessv1beta1.ClientConfig{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ClientConfig")
			Expect(testClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ClientConfigReconciler{
				Client: testClient,
